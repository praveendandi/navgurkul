...
def month_dates(doc, method=None):
    try:
        ...
        t.docstatus = 0 OR t.docstatus = 1 and t.employee='{emp}'""".format(emp=doc.employee), as_dict=True) 
        ...
        total_hours_value = total_hours_count(doc)
        doc.total_hours = total_hours_value
        ...
        if doc.workflow_state == "Reapply" and not doc.reason_for_reject:
            frappe.msgprint(f"🚨 Heyy 👩🏻‍💻!! The time sheet has been reapply and sent for resubmission -{doc.employee_name}!! 📣")
    ...
    if method == "on_submit" or doc.workflow_state == "Approve":
        ...
        while current_date <= end_date:
            ...
            if date_str not in total_hours_per_date and date_str not in employee_holidays:
                try:
                    create_attendance_record_dy_missing = frappe.get_doc({
                        "doctype": "Attendance",
                        "employee": doc.employee,
                        "attendance_date": date_str,
                        "docstatus": 1
                    })
                    create_attendance_record_dy_missing.status = "Absent"
                    create_attendance_record_dy_missing.insert()

                except Exception as e:
                    frappe.log_error(f"Error creating attendance for {date_str}: {e}")
        ...
        for date, total_hours in total_hours_per_date.items():
            if not frappe.db.exists("Attendance", {"employee": doc.employee, "attendance_date": date}):
                try:
                    ...
                    frappe.log_error(f"Error creating attendance for {date}: {e}")
            else:
                pass
